...
Framework.prototype.urlFor = function(path) {
  // TODO: Fix this it looks like a bug in NodeJS
  // return l.path.join(this.buildVersion, this.nameFor(path));
  return this.buildVersion + '/' + this.nameFor(path);
};
...
Framework.prototype.build = function(callback) {
  ...
  var buildScripts = function(files, callback) {
    ...
    if (that.validateScripts === true) {
      handlers.push('jslint');
    }
...
Framework.sproutcoreFrameworks = function(options) {
  ...
  if (this._sproutcoreFrameworks === undefined) {
    ...
    try { // SproutCore >= 1.5
      ...
      console.log('*** Building for SproutCore >= 1.5 ***');
      frameworks = ['jquery', 'bootstrap', 'runtime', 'handlebars', 'core_foundation', 'datetime', 'ajax', 'foundation', 'datastore', 'desktop'];
    } catch (e) { // SproutCore <= 1.4
      console.log('*** Building for SproutCore <= 1.4 ***');
      frameworks = ['runtime', 'foundation', 'datastore', 'desktop', 'animation'];
    }
    // Add 1.4.5-pre5 frameworks for now
    frameworks = ['jquery','runtime','handlebars', 'core_foundation','foundation', 'datetime', 'datastore', 'desktop', 'animation']; // 1.5.4-pre5
    // frameworks = ['jquery','runtime', 'foundation', 'datastore', 'desktop', 'animation']; // 1.5.4-pre5
